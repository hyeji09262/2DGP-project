...
def get_events():
    ...
    while SDL_PollEvent(ctypes.byref(sdl_event)):
        ...
        if event.type in (SDL_QUIT, SDL_KEYDOWN, SDL_KEYUP, SDL_MOUSEMOTION, SDL_MOUSEBUTTONDOWN, SDL_MOUSEBUTTONUP, SDL_MOUSEWHEEL):
            if event.type == SDL_QUIT:
                ...
                # wheel event does not have x,y position
                event.x, event.y = sdl_event.wheel.x, sdl_event.wheel.y
                events.append(event)
...
# for debugging draw
def draw_rectangle(x1, y1, x2, y2, r=255, g=0, b=0, a=255, filled=False):
    """
    Draws a rectangle from (x1, y1) to (x2, y2) with the specified color (r, g, b, a).
    Coordinates are in canvas space (origin at bottom-left).
    If filled=True, draws a filled rectangle; otherwise draws only the outline.
    """
...
def draw_line(x1, y1, x2, y2, r=0, g=0, b=0, a=255):
    """
    Draws a line from (x1, y1) to (x2, y2) with the specified color (r, g, b, a).
    Coordinates are in canvas space (origin at bottom-left).
    """
    SDL_SetRenderDrawColor(renderer, r, g, b, a)
    # Convert coordinates from canvas space (origin bottom-left) to SDL space (origin top-left)
    y1_sdl = -y1 + canvas_height - 1
    y2_sdl = -y2 + canvas_height - 1
    SDL_RenderDrawLine(renderer, int(x1), int(y1_sdl), int(x2), int(y2_sdl))

def draw_circle(x, y, radius, r=0, g=0, b=0, a=255, filled=False):
    """
    Draws a circle centered at (x, y) with the specified radius and color (r, g, b, a).
    Coordinates are in canvas space (origin at bottom-left).
    If filled=True, draws a filled circle; otherwise draws only the outline.
    """
    SDL_SetRenderDrawColor(renderer, r, g, b, a)
    # Convert y coordinate from canvas space to SDL space
    y_sdl = -y + canvas_height - 1
    center_x, center_y = int(x), int(y_sdl)

    if filled:
        # Draw filled circle using Bresenham's circle algorithm with horizontal line fills
        ...
        while dy >= dx:
            ...
            SDL_RenderDrawPoint(renderer, center_x - dx, center_y + dy)
            SDL_RenderDrawPoint(renderer, center_x - dx, center_y - dy)
            SDL_RenderDrawPoint(renderer, center_x + dy, center_y + dx)
            SDL_RenderDrawPoint(renderer, center_x + dy, center_y - dx)
            SDL_RenderDrawPoint(renderer, center_x - dy, center_y + dx)
            SDL_RenderDrawPoint(renderer, center_x - dy, center_y - dx)
